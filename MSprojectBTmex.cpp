...
// [phi1,phi2] = MSprojectBTmex(phi1,phi2,alpha,hz,max_iter,tol);
    ...
    //--------------------------------------------------------------------
    // Function
    long unsigned int lengthTmp1 = sizephi1[2]*sizephi1[2]*sizephi1[2];
    long unsigned int lengthTmp2 = sizephi1[0]*sizephi1[1]*sizephi1[2];
	...
	long unsigned int sizelastDiffPhi1[3] = {sizephi1[2],sizephi1[2],sizephi1[2]};
    ...
    for(int i=0; i < sizephi1[2]+1; i++) {
        DeltaS[i] = hz * (float)i;
        DeltaS[i] = 1.0f/DeltaS[i];
        powers[i] = pow(hz * (float)i,alpha - 1.0f);
    }
    ...
    for(int pixelx=0; pixelx<sizephi1[0]; pixelx++) {
        for(int pixely=0; pixely<sizephi1[1]; pixely++) {
            ...
            for(int i=0; i < lengthTmp; i++) {
				lastDiffPhi1[i] = 0; lastDiffPhi2[i] = 0;
			}
            ...
            while(iter <= max_iter && error>tol) {   
                ...
                for(int t1=0; t1<sizephi1[2]; ++t1) {
                    for(int t2=t1; t2<sizephi1[2]; ++t2) {                         
                        ...
                        beta = minimumvalueoftwo(0.0f, powers[lengthTmp]/(sqrt(A*A+B*B))-DeltaS[lengthTmp]);
                        // Project phiold
                        for(int i=0; i<lengthTmp; ++i) {
                            indexTmp1 = pixelx + pixely*sizephi1[0] + (t1+i)*sizephi1[0]*sizephi1[1];
